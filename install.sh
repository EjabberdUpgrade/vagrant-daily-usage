...
prerelease="false"
package_url="https://opscode-omnibus-packages.s3.amazonaws.com"
install_package="chef-server-${version}-${platform}-${platform_version}-${machine}"
...
deb_filename() {
  ...
  if [ "$machine" = "x86_64" ];
  then
    filename="chef-server_${version}_amd64.deb"
  else
    filename="chef-server_${version}_i386.deb"
  fi
}
...
rpm_filename() {
  filetype="rpm"
  filename="chef-server-${version}.${machine}.rpm"
}
...
svr4_filename() {
  ...
  filetype="solaris"
  filename="chef-server-${version}.${machine}.solaris"
}

# Set the filename for the sh archive
shell_filename() {
  filetype="sh"
  filename="chef-server-${version}-${platform}-${platform_version}-${machine}.sh"
}
...
url="https://opscode-omnibus-packages.s3.amazonaws.com/${platform}/${}/x86_64/${install_package}"
